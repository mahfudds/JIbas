 ...
 **[N]**/ ?>
<?php
function ShowSelectDepartemenTabungan($db)
{
    global $departemen;
...
function ShowDaftarJenisTabungan($db)
{
    ...
    try
    {
        ...
        while ($row = mysqli_fetch_array($request)) {
            $sql = "SELECT nama FROM jbsfina.rekakun WHERE kode = '$row[rekkas]'";
            $result = $db->QueryDb($sql);
            $row2 = mysqli_fetch_row($result);
            $namarekkas = $row2[0];

            $sql = "SELECT nama FROM jbsfina.rekakun WHERE kode = '$row[rekutang]'";
            $result = $db->QueryDb($sql);
            $row2 = mysqli_fetch_row($result);
            $namarekutang = $row2[0];
            ...
            echo "<strong>Hutang:</strong> $row[rekutang] $namarekutang <br />";
            echo "</td>";
    ...
    {
        echo Msg::InfoError($ex->getMessage(), "kbyhk");
    }
}
...
function ChangeAktif()
{
    ...
    {
        return json_encode([-99, Msg::InfoError($ex->getMessage(), "kntxr") ]);
    }
    finally
    {
        $db->Close();
...
function HapusJenisTabungan()
{
    ...
    try
    {
        ...
        $sql = "DELETE FROM jbsfina.datatabungan
                 WHERE replid = $id";
        $db->QueryDb($sql);
