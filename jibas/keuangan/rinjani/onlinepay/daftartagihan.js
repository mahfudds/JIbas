...
showTagihanData = function (nis, nama, noTagihan, idTagihanInfo) {
    ...
    $.ajax({
        ...
        success: function (data) {
            $("#dvTagihanData").html(data).hide().fadeIn(500);
...
{
    var qs = "op=modifinvoice2";
    qs += "&nomor=" + encodeURIComponent(execData2);

    if (execCode === 4) 
    {
        qs = "op=modifinvoiceset2";
        qs += "&idts=" + execData2;
    }
...
doHapusTagihanSet = function (no, idTagihanSet) 
{
    ...
    $.ajax({
        ...
        success: function (jsonResult) {
            ...
            sendToAppServer("datasync"); // 2025-05-25
...
changeTagihanSel = function () {
    ...
    $.ajax({
        ...
        success: function (data) 
        {
            $("#dvTagihanSet").html(data).hide().fadeIn(500);
            if ($("#tabTagihanSet").length)
                Tables('tabTagihanSet', 0, 0);
        },
        error: function (xhr) {
...
function showDaftarTagihanHelp() {
    $.ajax({
        url: "../help/op_daftartagihan.html?r=" + Math.random(),
