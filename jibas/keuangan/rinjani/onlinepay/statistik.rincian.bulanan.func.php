...
function ShowRincianStatistikBulanan()
{
    ...
    try
    {
        ...
        DoShowRincianStatistikBulanan($db);
    }
    catch (Exception $ex)
    {
        echo  Msg::InfoError($ex->getMessage(), "k7ppg");
    }
    finally
    {
        $db->Close();
...
function DoShowRincianStatistikBulanan($db)
{
    $page = 1;
...
<?php
    ...
    while($row = mysqli_fetch_array($res))
    {
        ...
        {
            $sql = "SELECT ts.nomor
                      FROM jbsfina.tagihansiswainfo2 tsi, jbsfina.tagihanset2 ts
...
function ShowRekapStatistikBulanan()
{
    ...
    {
        echo Msg::InfoError($ex->getMessage(), "k514u");
    }
    finally
    {
        $db->Close();
...
{
    global $bulan, $tahun, $departemen, $bankNo, $bankName, $idPetugas, $metode;
