...
<tr>
    <td>
<?php
        $sql = "SELECT DISTINCT p.replid, p.nis, s.nama AS namasiswa, p.bankno, p.nomor, p.jenis,
                       DATE_FORMAT(p.waktu, '%d %b %Y<br>%H:%i') AS fwaktu, DATE_FORMAT(p.tanggal, '%d %b %Y') AS ftanggal,
                       p.idpetugas, p.petugas, p.ketver, p.transaksi, p.ketver, b.bank, b.bankname, b.bankno
...
?>
        ...
        <table id="tabReport" border="1" class="tab" cellpadding="5" cellspacing="0">
        <tr style="height: 25px">
            <td class="header" width="35" align="center">No</td>
            <td class="header" width="180" align="center">Siswa</td>
            <td class="header" width="240" align="center">Jumlah</td>
            <td class="header" width="240" align="center">Bank/Petugas</td>
...
<?php
        ...
        while($row = mysqli_fetch_array($res))
        {
            ...
            echo "<td align='left' valign='top' colspan='3' style='position: relative'>";
            ...
            echo "<span class='bs_secondary fst_italic'>$row[nomor]</span>";
            echo "</td>";
            echo "<td width='20%' align='right'  style='border: none;' valign='top'>";
            echo "<span class='bs_secondary fst_italic'>$row[fwaktu]</span>";
            echo "</td>";
            echo "</tr>";
            echo "</table>";
            ...
            while($row2 = mysqli_fetch_array($res2))
            {
                ...
                echo "<td width='110px' align='right'><span class='money' style='font-size: 12px;'>$rp</span></td>";
                echo "<td width='110px' align='center'>";
                echo "<a style='cursor: pointer; color: #0000ff; text-decoration: underline; font-weight: normal;' onclick='showRincianJurnal($cnt)'>$row2[nokas]</a>";
                echo "</td>";
                echo "</tr>";
            }
            echo "</table>";
            ...
            echo "<strong>$row[namasiswa]</strong><br>";
            echo "<span class='bs_secondary'>$row[nis]</span>&nbsp;&nbsp;";
            echo "<img src='../images/ico/lihat.png' style='cursor: pointer;' title='lihat info siswa' onclick=\"showInfoSiswa('$row[nis]')\">";    
            echo "</td>";

            echo "<td align='right' valign='top'>";
            echo "<span class='fst_currency fst_big fst_bold'>$rpTransaksi</span><br><span class='bs_seconday'>$row[ketver]</span>";
            echo "</td>";

            echo "<td align='left' valign='top'>";
            echo "<b>$row[bank]</b><br>$row[bankname] - $row[bankno]<br>$row[petugas] - $row[idpetugas]";
            echo "</td>";
