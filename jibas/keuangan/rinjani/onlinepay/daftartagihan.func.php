...
function ShowTagihanSet($db)
{
    ...
    $no = 0;
    echo "<table id='tabTagihanSet' class='tab' cellpadding='2' cellspacing='0' style='width: 390px;'>";
    while($row = mysqli_fetch_array($res))
    {
        ...
        $sql = "SELECT COUNT(replid)
                  FROM jbsfina.tagihansiswainfo2
        ...
        echo "<span class='bs_secondary' style='font-style: italic;'>$row[nomor]&nbsp;&nbsp;|&nbsp;&nbsp;$row[ftanggal]</span>";
        ...
        echo "</td>";
        echo "<td style='background-color: #ffe9e1;font-size: 12px;' width='*' align='center'>";
...
function DaftarTagihanInfo()
{
    ...
    try
    {
        ...
        $sql = "SELECT t.replid, t.nis, s.nama, t.info, t.jumlah, t.status, t.notagihan,
                  ...
                  FROM jbsfina.tagihansiswainfo2 t, jbsakad.siswa s
                 WHERE t.nis = s.nis
        ...
        echo "<span class='bs_secondary fst_italic'>$infoSet</span>";
        ...
        echo "<table id='tabTagihanInfo' class='tab' cellpadding='2' cellspacing='0' style='width: 390px;'>";
        $nSiswa = 0;
        while($row = mysqli_fetch_array($res))
        {
            $nSiswa += 1;
            ...
            echo "<input type='checkbox' id='chsiswa-$nSiswa'><b>" . $row["nama"] . "</b><br><span class='bs_secondary'>" . $row["nis"] . "</span>";
            echo "<input type='hidden' id='nis-$nSiswa' value='$nis'>";
            echo "</td>";
            echo "<td style='width: 230px' valign='top' align='left'>";
            echo "<span class='fst_currency fst_bold'>" . FormatRupiah($jumlah) . "</span><br><span class='bs_secondary fst_italic'>" . $row["notagihan"] . "</span>";
            echo "</td>";
            echo "</tr>";
        }
        echo "</table><br>";
        ...
        if ($nSiswa > 0)
        {
            ...
            echo "<input type='button' class='dialogButtonPositive' value='Kirim Pesan Tagihan' style='height: 30px' onclick='prepareBatchNotif()'>";
        }
    }
    catch (Exception $ex)
    {
        Logger::LogErrorOnce($ex, "k0s7s");
...
function ShowPrepareBatchNotif()
{
    ...
    try
    {
        ...
        echo "<input type='button' id='pt_kirim' class='dialogButtonPositive' style='height: 30px; width: 60px' value='Kirim' onclick='sendBatchNotif()'>";
        echo "&nbsp;&nbsp;<span style='color: #0000ff' id='pt_info'></span>";
        echo "</div>";
    }
    catch (Exception $ex)
    {
        Logger::LogErrorOnce($ex, "kms4g");
...
function SendNotif()
{
    ...
    try
    {
        ...
        $success = 0;
        CreateSMSTunggakan2($db, "SISPAY", $dept, $nis, $nama, $pesan, $success);

        echo "[1,\"Berhasil menyiapkan pesan tagihan\"]";
    }
    catch (Exception $ex)
    {
        Logger::LogErrorOnce($ex, "kvqjb");
...
function SendBatchNotif()
{
    ...
    try
    {
        ...
        $sql = "SELECT ts.stiuran, t.jumlah, t.nis, s.nama, ts.bulan, ts.tahun
                  FROM jbsfina.tagihansiswainfo2 t
                 INNER JOIN jbsakad.siswa s ON t.nis = s.nis
                 INNER JOIN jbsfina.tagihanset2 ts ON t.idtagihanset = ts.replid
...
function DaftarTagihanData()
{
    ...
    try
    {
        ...
        echo "<span class='fsz_18 fw-bold'>$nama</span><br><span class='fsz_14 bs_secondary'>$nis</span>";
        echo "&nbsp;&nbsp;<img src='../images/ico/lihat.png' onclick='showInfoSiswa(\"$nis\")' style='cursor: pointer;' title='lihat info siswa'>";
        echo "<br><br>";
        ...
        echo "<td align='left' width='500' style='font-size: 12px'>";
        echo "<b>$noTagihan</b><br>";
        echo "<span class='bs_secondary fst_italic'>$nomorTs</span>";
        echo "</td>";
        echo "</tr>";
        echo "<tr>";
        echo "<td align='right' style='font-size: 12px'>Tagihan:</td>";
        echo "<td align='left' style='font-size: 12px'>$row[info]</td>";
        echo "</tr>";
        echo "<tr>";
        echo "<td align='right' style='font-size: 12px'>Jumlah:</td>";
        $jumlah = $row['jumlah'];
        echo "<td align='left'><span class='fst_currency' style='font-size: 14px; font-weight: bold;'> " . FormatRupiah($jumlah) . "</span></td>";
        echo "</tr>";
        echo "<tr>";
        ...
        echo "<table class='tab' id='tabTagihanData' cellpadding='5' cellspacing='0'>";
        ...
        while($row = mysqli_fetch_array($res))
        {
            ...
            echo "<tr style='height: 25px;'>";
            echo "<td align='center' class='bg_no' valign='top'>$no</td>";
        ...
        if ($status == 0)
            echo "<input type='button' id='btHapusTagihanSiswa' class='dialogButtonNegative' style='width: 170px; height: 30px;' value='Hapus Tagihan Siswa' onclick='hapusTagihanSiswa(\"$noTagihan\")'>";
        ...
        echo "<div style='width: 600px'>";
        echo "<input type='button' id='ps_kirim' class='dialogButtonPositive' style='height: 30px; width: 60px' value='Kirim' onclick='sendNotif()'>";
...
function HapusTagihanData()
{
    ...
    try
    {
        ...
        $sql = "DELETE FROM jbsfina.tagihansiswadata2
...
function HapusTagihanSiswa()
{
    ...
    try
    {
        ...
        $sql = "DELETE FROM jbsfina.tagihansiswainfo2
                 WHERE notagihan = '$noTagihan'";
        $db->QueryDb($sql);

        /*
        $sql = "DELETE FROM jbsfina.tagihansiswalayanan2
                 WHERE notagihan = '$noTagihan'";
        QueryDbEx($sql);
        */
        $db->CommitTrans(); 
...
function HapusTagihanSet()
{
    ...
    try
    {
        ...
        */
        ...
        $sql = "DELETE FROM jbsfina.tagihanset2
