...
{
    if ($metode == 1)
        return "Tagihan Sekolah";

    if ($metode == 2)
        return "Karanjang Pembayaran";
...
function NamaPenerimaan2($db, $kategori, $idPenerimaan)
{
    if ($idPenerimaan == "0")
    {
        if ($kategori == "BL")
...
function MidtransPaymentInformation($paymentType, $paymentSource)
{
    ...
    if ($paymentType == "credit_card")
        return "Credit Card " . strtoupper($paymentSource);

    if ($paymentType == "cstore")
        return strtoupper($paymentSource);

    if ($paymentType == "bank_transfer")
        return strtoupper($paymentSource);

    return strtoupper($paymentType) . " " . strtoupper($paymentSource);
}
