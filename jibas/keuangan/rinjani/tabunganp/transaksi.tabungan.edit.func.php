...
function LoadValues($db)
{
    ...
    $sql = "SELECT pg.nip, pg.nama, p.idjurnal, p.debet, p.kredit, DATE_FORMAT(p.tanggal, '%d-%m-%Y') AS tanggal, 
              ...
              FROM jbsfina.tabunganp p, jbssdm.pegawai pg, jbsfina.datatabunganp pn 
...
function SimpanEditTabungan()
{
    ...
    catch (Exception $ex)
    {
        ...
        return json_encode([-99, Msg::InfoError($ex->getMessage(), "kds9p")]);
    }
    finally
    {
        $db->Close();
...
function ShowSelectLokasiPengambilanTabunganPegawai($db, $nip, $idTabungan, $defLokasiDana)
{
    try
    {
