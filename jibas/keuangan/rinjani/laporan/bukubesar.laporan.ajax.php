...
if ($op == "daftar")
{
    ...
    try
    {
        ...
        $kategori = RequestData("kategori", "nokas");
        $koderek = RequestData("koderek", "nokas");
        $namarek = RequestData("namarek", "nokas");
        $page = RequestData("page", 1);
        $urut = RequestData("urut", "nokas");

        ShowTransaksiBukuBesar($db);
    }
    catch (Exception $ex)
    {
        echo Msg::InfoError($ex->getMessage(), "kvt62");
    }
    finally
    {
        $db->Close();
