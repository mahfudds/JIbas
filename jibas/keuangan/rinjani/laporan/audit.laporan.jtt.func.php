...
function ShowLaporanAuditJtt($db)
{
    global $nRowPerPage;
    global $idTahunBuku, $departemen, $lap, $tanggal1, $tanggal2, $page;

    $auditTable = "jbsfina.auditpenerimaanjtt";
    if ($lap == "penerimaanjttcalon")
        $auditTable = "jbsfina.auditpenerimaanjttcalon";
    ...
    echo "<td class='header' width='15%'>Diskon</td>";
    ...
    $sql = "SELECT DISTINCT ai.petugas as petugasubah, j.transaksi, date_format(ai.tanggal, '%d-%b-%Y %H:%i:%s') as tanggalubah, ap.replid AS id, 
              ...
              FROM $auditTable ap, jbsfina.auditinfo ai, jbsfina.jurnal j 
    ...
    while ($row = mysqli_fetch_array($result))
    {
        ...
        if ($cnt % 2 == 0)
        {
            ...
            echo "<td colspan='6' align='left' style='background-color: #3994c6; color: #ffffff;'><em><strong>Perubahan dilakukan oleh $row[petugasubah] tanggal $row[tanggalubah] </strong></em></td>";
            echo "</tr>";
            echo "<tr>";
            echo "<td colspan='6' style='background-color: #e5fdff;'><strong>No. Jurnal :</strong> $row[nokas] ";
        ...
        echo "<td align='right'><b>" . FormatRupiah($row['diskon']) . "</b></td>";
        ...
        {
            echo "<tr>";
            echo "<td colspan='7' style='height: 2px'></td>";
            echo "</tr>";
        }
    }
    echo "</table>";
