...
function ShowTagihanReport($showMenu)
{
    ...
    try
    {
        ...
        $sb->AppendLine("<td width='180' rowspan='2' align='center' class='header-bgonly'>Vendor</td>");
        $width = $payWidth * $nDept;
        $sb->AppendLine("<td width='$width' colspan='$nDept' align='center' class='header-bgonly'>Siswa</td>");
        $sb->AppendLine("<td width='$payWidth' align='center' class='header-bgonly'>Pegawai</td>");
        $sb->AppendLine("<td width='$payWidth' rowspan='2' align='center' class='header-bgonly'>Sub Total</td>");
        $sb->AppendLine("</tr>");
        $sb->AppendLine("<tr style='height: 20px'>");
        for($i = 0; $i < $nDept; $i++)
        {
            $dept = $lsDept[$i];
            $sb->AppendLine("<td width='$payWidth' align='center' class='header-bgonly'>$dept</td>");
        }
        $sb->AppendLine("<td width='$payWidth' align='center' class='header-bgonly'>$deptPeg</td>");
        $sb->AppendLine("</tr>");
        ...
        for($i = 0; $i < count($lsVendor); $i++)
        {
            ...
            $no += 1;
            $sb->AppendLine("<tr style='height: 30px'>");
            $sb->AppendLine("<td align='center' valign='middle' rowspan='2' class='numberColumn'>$no</td>");
        ...
        $sb->AppendLine("</table>");
        $sb->AppendLine("<i>*) Tagihan dihitung dari penerimaan vendor selain pembayaran iuran sekolah oleh siswa</i>");

        echo $sb->ToString();
    }
    catch (Exception $ex)
    {
        Msg::InfoError($ex->getMessage(), "k3t5e");
    }
    finally
    {
        $db->Close();
