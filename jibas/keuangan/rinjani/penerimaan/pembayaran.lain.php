...
require_once('pembayaran.lain.func.php');
...
<head>
    ...
    <script language="javascript" src="pembayaran.lain.js?r=<?=filemtime('pembayaran.lain.js')?>"></script>
</head>
<body style="background-color: #efefef; padding: 5px;">

<?php
$payInfo = GetPaymentInfo($db, $idpenerimaan, $idtahunbuku);
if ($payInfo->Exist == false)
{
    echo "<i>Tidak ditemukan data penerimaan /kcuzj</i>";
    exit();
}
?>
...
<?php
                ShowRiwayatPembayaranLain($db, $payInfo->IdPenerimaan, $payInfo->IdTahunBuku);
?>
            </div>
