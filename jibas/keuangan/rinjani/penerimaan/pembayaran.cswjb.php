...
<head>
    ...
    <title>Pembayaran Iuran Wajib Calon Siswa</title>
    ...
    <script language="javascript" src="pembayaran.cswjb.js?r=<?=filemtime('pembayaran.cswjb.js')?>"></script>
</head>
<body style="background-color: #efefef; padding: 5px;">

<?php
$userInfo = UserInfo::CalonSiswa($db, $userId);
if ($userInfo->Exist == false)
{
    echo "<i>Tidak ditemukan data calon siswa $userId /khnck</i>";
    exit();
}

$payInfo = GetPaymentInfoCsWjb($db, $idpenerimaan, $idtahunbuku, $userInfo->IdCalonSiswa);
...
<input type="hidden" id="idcalonsiswa" value="<?= $userInfo->IdCalonSiswa ?>">

<div id="divSectionUser">
<?php
    UserInfo::ShowCalonSiswaAvatar($userInfo); ?>
</div>
