...
function ShowSelectDept($db)
{
    ...
    echo "<select name='departemen' id='departemen' class='inputbox' style='width:120px; font-size: 14px;' onchange='change_dep();'>";
...
function GetSelectPayment($departemen, $kelompok)
{
    ...
    try
    {
        ...
        if ($num == 0)
        {
            $arrJson = array(0, "Belum ada konfigurasi batch payment. Atur dahulu di menu <b>Pengaturan Batch Payment</b>", "");
        }
        else
        {
            $arrKet = array();
            ...
            $idFirst = 0;
            $data = "<select id='paymentSelect' onchange='changePayment()' class='inputbox' style='width: 350px; font-size: 14px;'>";
    ...
    catch (Exception $ex)
    {
        ...
        $arrJson = array(0, Msg::InfoError($ex->getMessage(), "ka97j"), "");
        echo json_encode($arrJson);
    }
    finally
    {
...
function GetPaymentList($idAutoTrans, $kelompok, $noid, $idTahunBuku)
{
    ...
    try
    {
        ...
        else
        {
            ...
            $tab .= "</td></tr></table>";
            $tab .= "<table id='tabDaftar' border='1' class='tab' cellpadding='5' style='border-collapse: collapse; border-width: 1px;' cellspacing='0' width='1100'>";
            ...
            while($row = mysqli_fetch_row($res))
            {
                ...
                $tab .= "<tr>";
                $tab .= "<td align='center' class='numberColumn'>$no</td>";
                ...
                $element = "bayar-$no";
                $tab .= "<input type='text' $disabled $readonly id='$element' name='$element' value='$fcicilan' class='inputbox-money' style='width: 115px; text-align: right;' onblur=\"calculatePay(); formatRupiah('$element');\" onfocus=\"unformatRupiah('$element')\">";
                $tab .= "</td>";
                $tab .= "<td align='center' valign='top'>";
                $element = "diskon-$no";
                $tab .= "<input type='text' $disabledDiskon $disabled $readonly id='$element' name='$element' value='$diskon' class='inputbox-money' style='width: 115px; text-align: right;' onblur=\"calculatePay(); formatRupiah('$element');\" onfocus=\"unformatRupiah('$element')\">";
                $tab .= "</td>";
                $tab .= "<td align='left' valign='top'>";
                $element = "keterangan-$no";
                $tab .= "<input type='text' $disabled $readonly id='$element' name='$element' value='' class='inputbox' style='width: 330px'><br>";
                $tab .= "<i>$informasi</i>";
                $tab .= "</td>";
                $tab .= "</tr>";
            }
            ...
            if ($validTrans)
            {
                ...
                $tab .= "<textarea id='ktransaksi' name='ktransaksi' class='inputbox' rows='2' cols='50'></textarea>";

                $tab .= "</td>";
                $tab .= "<td valign='middle'>";
                $tab .= "<input type='submit' class='dialogButtonPositive' value='Simpan Pembayaran' style='height: 40px; width: 160px;'>";
                $tab .= "</td>";
                $tab .= "<td valign='middle'>";
                $tab .= "<input type='button' class='dialogButtonNegative' value='Tutup' onclick='tutupPembayaran()' style='height: 40px; width: 60px;'>";
                $tab .= "</td></tr>";
                $tab .= "</table>";
            }
            else
    ...
    catch (Exception $ex)
    {
        ...
        $arrJson = array(0, Msg::InfoError($ex->getMessage(), "k94hb"), "");
        echo json_encode($arrJson);
    }
    finally
    {
...
function ShowAccYear($db)
{
    ...
    if (mysqli_num_rows($result) > 0)
    {
        ...
        echo "<input type='text' name='tahunbuku' id='tahunbuku' class='inputbox inputbox-readonly' readonly style='width: 200px; font-size:14px;' value='" . $row['tahunbuku'] . "'/>";
        echo "<input type='hidden' name='idtahunbuku' id='idtahunbuku' value='" . $row['id'] . "'/>";
    }
    else
    {
        echo "<input type='text' name='tahunbuku' id='tahunbuku' class='inputbox inputbox-readonly' readonly style='width: 200px; font-size:14px;' value=''/>";
