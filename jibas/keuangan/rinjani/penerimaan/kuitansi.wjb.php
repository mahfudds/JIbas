...
if ($status == "calon")
{
    $sql = "SELECT p.replid AS id, p.idbesarjttcalon, b.besar, c.nopendaftaran, c.nama, j.nokas, 
			  ...
			  FROM jbsfina.penerimaanjttcalon p, jbsfina.besarjttcalon b, jbsfina.jurnal j, jbsakad.calonsiswa c 
...
else
{
    $sql = "SELECT p.replid AS id, p.idbesarjtt, b.besar, b.nis, s.nama, j.nokas, 
				   j.transaksi, date_format(p.tanggal, '%d-%b-%Y') as tanggal, p.keterangan, p.jumlah, 
				   p.petugas, j.idtahunbuku, p.info1 AS diskon 
			  FROM jbsfina.penerimaanjtt p, jbsfina.besarjtt b, jbsfina.jurnal j, jbsakad.siswa s 
...
if ($status == "calon")
{
    ...
    $sqlSum = "SELECT SUM(jumlah), SUM(info1) 
...
$result = $db->QueryDb($sqlSum);
$row = @mysqli_fetch_row($result);
$jumlahbayar = $row[0];
$jumlahdiskon = $row[1];

$sql = "SELECT departemen FROM jbsfina.tahunbuku WHERE replid='$idtahunbuku'";
$result = $db->QueryDb($sql);
...
<?php                   getSmallHeaderLogo2($db, $departemen); ?>
                    </td>
                    <td align="left">
                    ...
                    <td align="left" colspan='2'>
                        ...
                        <table border="0" cellpadding="2" cellspacing="0" width="100%">
                            <tr>
                                <td width="20">&nbsp;</td>
                                <td width="60">
<?php                               if ($_REQUEST["status"] == "calon") echo  "No Pendaftaran"; else echo  "N I S"; ?> </td>
