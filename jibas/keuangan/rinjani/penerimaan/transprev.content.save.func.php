...
{
    global $studentid, $studentname, $idtahunbuku, $idtahunbukuaktif;
    global $transactions;
    ...
    if ($success)
    {
        $nokas = GetNoKas($idtahunbukuaktif);

        $transactions[] = array($nokas, $infocicil, $jcicilan, $jdiskon, "JTT", $tersisa);

        $success = SimpanJurnal($idtahunbukuaktif, $tcicilan, $infociciljurnal, $nokas, "", $idpetugas, $petugas, "penerimaanjtt", $idjurnal);
    }
    ...
    if ($success)
    {
        $sql = "UPDATE tahunbuku SET cacah=cacah+1 WHERE replid='$idtahunbukuaktif'";
        //Logger::LogOnce($sql);
...
function SaveIuranWajibCalonSiswa($rowno)
{
    ...
    if ($success)
    {
        ...
        $success = SimpanJurnal($idtahunbukuaktif, $tcicilan, $infociciljurnal, $nokas, "", $idpetugas, $petugas, "penerimaanjttcalon", $idjurnal);
    }
...
{
    global $studentid, $idtahunbukuaktif, $ktransaksi;
    global $transactions;
    ...
    $sql = "INSERT INTO jbsfina.multitransinfo
               SET idtahunbuku = $idtahunbukuaktif, $userCol = '$studentid', tanggal = NOW(), 
